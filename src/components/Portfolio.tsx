...
export const Portfolio: React.FC<{ id: string, localOverrides: any, isPreview?: boolean }> = ({ id, localOverrides: overrides, isPreview = true }) => {
            ...
            <div className="max-w-7xl mx-auto" style={{ paddingTop: `${layout.paddingY}px`, paddingBottom: `${layout.paddingY}px` }}>
                ...
                >
                    {(data.items || []).map((item: any, idx: number) => {
                        ...
                        return hasLink ? (
                            <a key={item.id} href={item.link} target="_blank" rel="noopener noreferrer" onClick={handleLinkClick}>
                                {content}
                            </a>
                        ) : content;
                    })}
